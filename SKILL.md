...
**在做任何动作(包括 STEP 1)前,必须先探测环境能力**。不能凭"经验"或"直觉"假设 CC 不存在。

### 0.5.1 必跑的探测命令(Bash,每条命令分开跑)
...
- ❌ 未跑探测命令就说"CC 不可用"
- ❌ 基于"上次对话里用户没提"或"我的知识里该用户没装"假设环境
- ❌ 在 `delegate_task` / 内部 spawn 这类封装里"以为"启了 CC,但不验证是否真走了 `claude-code` 命令
...
## STEP 2.5 进入 STEP 3 前的硬校验(防止绕过 feature 启动)

**在修改任何 feature 的 `status` 为 `in_progress` 或 `phase >= 2` 之前**,必须跑以下 Bash 命令全部通过。任一失败立即停止,报告用户。
...
- `feature.phase`: `1 | 2 | 3 | 4`(phase=0 只在 sprint 级,不在 feature 级)
...
**写入任何不在上述列表的值都算违规**。例如 `mode="executing"` / `status="done"` / `decision="pass"` 都是错的。

### Z11. DAG gate 必走

phase=1 plan 产出 feature DAG 后,**必须**推 gate checkpoint `cp-sprint-dag` 给用户确认,用户 approved 后才能进入 STEP 2.5 / STEP 3。

- ❌ 不推 DAG gate 直接推进 feature 到 phase=2 → 违规
- ❌ `eval_log.jsonl` 里无 `cp-sprint-dag` 条目但 feature_list 有 in_progress feature → 违规

### Z12. 封装 spawn 的诚实性

如果你(Hermes)有内部机制(如 `delegate_task`)可以"spawn" 子进程,**不等于**它能替代 STEP 3 的 Claude Code session 启动。

判断是否真启了 CC session 的唯一标准是**文件系统证据**:

- `$HARNESS_ROOT/.harness/features/<fid>/` 目录存在
- 该目录下 5 个必需文件齐全(context_snapshot / AGENTS / generator_log / handoff / session_id.txt)
- `.claude/hooks/*.sh` 4 个 + `settings.json` 部署完成
- `session_id.txt` 非空(被 CC 进程写入过)
- `session_pool.active.<fid>` 非空

以上**任一**不满足,**不得**声称"已启 CC"/"已 delegate",**不得**修改 feature.status 为 in_progress。

如果你的 `delegate_task` 不生成这些文件,它就**不符合**本 skill 的 feature 启动协议,**禁止**用它代替 STEP 3.3 的 `claude-code --cwd $FDIR` 命令。

### Z13. 能力探测诚实性

STEP 0.5 环境探测必须**真跑 bash 命令**,不得:

- ❌ 基于"对话历史"或"用户之前没说"推测环境
- ❌ 跳过探测直接声称"CC 不可用"
- ❌ `environment_probe.json` 里填 `uncertain` 却继续推进
